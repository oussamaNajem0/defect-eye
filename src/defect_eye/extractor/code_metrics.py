...
class FunctionMetrics:
    """Dataclass holding function-level metrics."""
...
@dataclass
class FileMetrics:
    """Dataclass holding aggregated file-level metrics."""
...
def analyze_file(file_path: str | Path) -> Optional[FileMetrics]:
    """Parse a single file using Lizard and calculate aggregated metrics.

    Args:
        file_path: Path to the source file.

    Returns:
        FileMetrics object or None if the file cannot be parsed.
    """
    path = Path(file_path)
    if not path.is_file():
...
def file_metrics_to_dict(metrics: FileMetrics) -> Dict[str, Any]:
    """Convert FileMetrics dataclass to dictionary for dataset export."""
